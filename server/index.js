...
/*app.get('*', (req, res) => {
    ...
    Q.fcall(() =>
        fs.readdirSync("./lol/").length ?
        fs.readdirSync("./lol/") :
        res.sendStatus(500)
    )
    .then(data => {
        let content = [];
        data.sort((file, nextfile) => {
            if (fs.statSync('./lol/' + file).birthtime > fs.statSync('./lol/' + nextfile).birthtime) return -1
            if (fs.statSync('./lol/' + file).birthtime < fs.statSync('./lol/' + nextfile).birthtime) return 1
            return 0
        }).forEach((file, index) =>
            content.push({
                id: index,
                createdate: fs.statSync('./lol/' + file).birthtime,
                editdate: fs.statSync('./lol/' + file).ctime,
                data: Marked(fs.readFileSync('./lol/' + file).toString())
            })
        )
        return content;
    })
    .then(data => {return res.status(200).json({posts: data})})
    .catch(err => {console.log(err); return res.sendStatus(500)})
    .done()
})
